...
void ListP::Insert(int position, double item) {
...
void ListP::Retrieve(double item) {
	Node* iter = this->Head;
	int count = 0;
	cout << endl;
...
bool ListP::SearchIndex(int index, double* outItem) {
	assert(outItem != NULL); // NULL love
	if (index > Count) {
		cout << "out of lange" << endl;
		return false;
	}
	if (index == 0) {
		*outItem = this->Head->Data;
		return true;
	}
	Node* iter = Head;
